    ...
    float* top_box, float* top_pose, float* top_target, float* top_weight, int* top_domain, int& num_rois, cudaStream_t stream)
{
  ...
  // PRINT(num_gt)
  // PRINT(batch_size)
  // PRINT(inlierThreshold)
  // PRINT(labelThreshold)
  // PRINT(votingThreshold)
  // PRINT(perThreshold)
  // PRINT(skip_pixels)
...
int resize_outputs(at::Tensor& top_box, at::Tensor& top_pose, at::Tensor& top_target, at::Tensor& top_weight, 
    at::Tensor& top_domain, const int num_rois, const int num_classes)
{
  top_box.resize_({num_rois, 7});
  top_pose.resize_({num_rois, 7});
  top_target.resize_({num_rois, 4 * num_classes});
  top_weight.resize_({num_rois, 4 * num_classes});
  
  top_domain.resize_({num_rois});

  return 1;
}

int reset_outputs(at::Tensor& top_box, at::Tensor& top_pose, at::Tensor& top_target, at::Tensor& top_weight, 
    at::Tensor& top_domain)
{
  top_box.zero_();
  top_pose.zero_();
  top_target.zero_();
    ...
    const int skip_pixels) 
{
  ...
  int batch_size = labelmap.size(0);
  int height = labelmap.size(1);
  int width = labelmap.size(2);
  ...
  // float tensors
  at::Tensor top_box = at::zeros({max_allowed_rois, 7}, vertmap.options());
  at::Tensor top_pose = at::zeros({max_allowed_rois, 7}, vertmap.options());
  at::Tensor top_target = at::zeros({max_allowed_rois, 4*num_classes}, vertmap.options());
  at::Tensor top_weight = at::zeros({max_allowed_rois, 4*num_classes}, vertmap.options());
  // int tensor
  at::Tensor top_domain = at::zeros({max_allowed_rois}, labelmap.options());
  ...
  {
    AT_DISPATCH_FLOATING_TYPES(vertmap.type(), "ROIAlign_backward", [&] {
      HoughVotingForwardLaucher(
            labelmap.contiguous().data<int>(), vertmap.contiguous().data<float>(), extents.contiguous().data<float>(), 
            meta_data.contiguous().data<float>(), gt.contiguous().data<float>(),
